...
test("empty research form is centered with a typewriter prompt", async () => {
  ...
  assert.match(app, /mx-auto flex min-h-full w-full max-w-\[1480px\] flex-col/, "the content surface should keep a readable maximum width while centering vertically");
  ...
  assert.doesNotMatch(form, /Markdown \/ LaTeX \/ 化学式/, "the form header should not show format capability text");
  assert.doesNotMatch(form, /\{question\.length\} \/ 8000/, "the character counter should live inside the input editor");

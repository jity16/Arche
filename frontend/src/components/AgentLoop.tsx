...
function Node({ label, Icon, status }: { label: string; Icon: ComponentType<{ className?: string }>; status: Step }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1 text-center">
      <div className={`relative flex size-9 items-center justify-center rounded-lg border transition-colors duration-300 ${tone}`}>
        {status === "done" ? <Check className="size-5" /> : <Icon className={`size-4 ${status === "running" ? "arche-pulse" : ""}`} />}
        {status === "running" && (
          <span className="absolute inset-[-3px] animate-spin rounded-lg border-2 border-amber-400 border-t-transparent" />
        )}
      </div>
...
const Connector = ({ on }: { on: boolean }) => (
  <div className="mx-1 mt-5 h-0.5 w-4 shrink-0 rounded-full" style={{ background: on ? "#14532d" : "#e2e8f0" }} />
);
  ...
  return (
    <div>
      ...
      {state.rounds.length === 0 ? (
        <div className="mt-2 rounded-lg border border-dashed border-slate-200 bg-slate-50 px-3 py-4 text-center text-xs text-slate-400">
          {idle ? "等待提交" : "等待进入反馈循环…"}
        </div>
      ) : (
        <div className="mt-2 space-y-2">
          {state.rounds.map((r) => (
            <div key={r.round} className="rounded-lg border border-slate-200 bg-white p-3">
              <div className="mb-2 flex items-center justify-between">
                ...
                {r.revised && (
                  <span className="inline-flex items-center gap-1 rounded-md bg-amber-50 px-2 py-0.5 text-[10px] font-medium text-amber-700 ring-1 ring-inset ring-amber-500/20">
      ...
      {/* 当前活动 */}
      <p className={`mt-3 text-center text-xs ${running ? "text-amber-600" : state.finished ? "text-[#14532d]" : "text-slate-400"}`}>
